...
from .matrix_vectorizer import MatrixVectorizer
...
def load_dataset(config):
    ...
    if config.dataset.name == 'csv':
        ...
        if len(target_mat_all) > n_samples:
            target_mat_all = target_mat_all[:n_samples]

    # Generate Erdos-Renyi graphs
    elif config.dataset.name == 'er':
...
def load_csv_graphs(csv_path: str, n_nodes: int):
    """
    从 CSV 读取每行的 vectorized 邻接矩阵，并用 MatrixVectorizer.anti_vectorize 还原为 (n_nodes, n_nodes) 矩阵。
