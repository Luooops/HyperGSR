...
def load_data(
    stp_gsr_path=DEFAULT_STP_PATH,
    hyper_edgeattr_path=DEFAULT_EDGEATTR_PATH,
    hyper_coord_path=DEFAULT_COORD_PATH,
):
    """Load metric CSVs in STP, HyperGSR EdgeAttr, HyperGSR Coord order."""
    # Load data
...
def create_bar_plots(data=None, *, output_dir=".", show=True):
    """Create comprehensive bar plots for all metrics"""
    stp_data, hyper_edgeattr_data, hyper_coord_data = load_data() if data is None else data
...
def create_improvement_heatmap(data=None, *, output_dir=".", show=True):
    ...
    plt.tight_layout()
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    plt.savefig(Path(output_dir) / 'improvement_heatmap.png', dpi=300, bbox_inches='tight')
    if show:
        plt.show()
    else:
        plt.close()

def create_radar_chart(data=None, *, output_dir=".", show=True):
    ...
    plt.tight_layout()
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    plt.savefig(Path(output_dir) / 'radar_chart.png', dpi=300, bbox_inches='tight')
    if show:
        plt.show()
    else:
        plt.close()

def create_summary_table(data=None, *, output_dir=".", show=True):
...
def main(data=None, *, output_dir=".", show=True):
    """Main function to generate all plots"""
    print("Generating comparison plots...")

    # Create all visualizations
    print("1. Creating bar plots...")
    create_bar_plots(data=data, output_dir=output_dir, show=show)

    print("2. Creating improvement heatmap...")
    create_improvement_heatmap(data=data, output_dir=output_dir, show=show)

    print("3. Creating radar chart...")
    create_radar_chart(data=data, output_dir=output_dir, show=show)

    print("4. Creating summary table...")
    summary_df = create_summary_table(data=data, output_dir=output_dir, show=show)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compare STP/EdgeAttr/Coord metrics")
    parser.add_argument("--stp_path", default=DEFAULT_STP_PATH)
    ...
    parser.add_argument("--no_show", action="store_true", help="Save figures without opening windows")
